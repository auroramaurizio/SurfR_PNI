...
fdrUP_GeneID_cells <- df_GEO_cells[df_GEO_cells$padj < 0.05 & df_GEO_cells$log2FoldChange > 1, "GeneID"]
SPup_GEO_cells <- Gene2SProtein(genes = fdrUP_GeneID_cells, input_type = "gene_name")
#443 out of 1832 genes have a matching surface protein

fdrDW_GeneID_cells <- df_GEO_cells[df_GEO_cells$padj < 0.05 & df_GEO_cells$log2FoldChange < 1, "GeneID"]
SPdw_GEO_cells <- Gene2SProtein(genes = fdrDW_GeneID_cells, input_type = "gene_name")
#302 out of 3567 genes have a matching surface protein


write.xlsx(SPup_GEO_cells, "Supp_table_GSE90711_SPup_GEO_cells.xlsx")
```
...
fdrUP_GeneID_fascicle <- df_GEO_fascicle[df_GEO_fascicle$padj < 0.05 & df_GEO_fascicle$log2FoldChange > 1, "GeneID"]
SPup_GEO_fascicle <- Gene2SProtein(genes = fdrUP_GeneID_fascicle, input_type = "gene_name")
#203 out of 1373 genes have a matching surface protein

fdrDW_GeneID_fascicle <- df_GEO_fascicle[df_GEO_fascicle$padj < 0.05 & df_GEO_fascicle$log2FoldChange < 1, "GeneID"]
SPdw_GEO_fascicle <- Gene2SProtein(genes = fdrDW_GeneID_fascicle, input_type = "gene_name")
#363 out of 2920 genes have a matching surface protein

SPup_GEO_fascicle

write.xlsx(SPup_GEO_fascicle, "Supp_table_GSE90711_SPup_GEO_fascicle.xlsx")
```
...
pdf("Enrichment_barplot_GEO_cells_UP_allgenes.pdf", 10, 4)
...
pdf("Enrichment_barplot_GEO_fascicle_UP_allgenes.pdf", 10, 4)
...
# ML only predicted SP in the GEO cells dataset - enrichment

```{r ML only predicted SP }
#library(enrichR)

ML_SPup_GEO_cells <- SPup_GEO_cells[grepl("machine learning", SPup_GEO_cells$Surfaceome.Label.Source),]
ML_SPup_GEO_cells <- ML_SPup_GEO_cells$UniProt.gene
df_SP_GEO_cells_genes_ML <- df_GEO_cells[rownames(df_GEO_cells) %in% ML_SPup_GEO_cells, ]
df_SP_GEO_cells_genes_ML <- list(SP_GEO_cells_ML = as.data.frame(df_SP_GEO_cells_genes_ML)) 

Enrich_GEO_BP_ML <- Enrichment(df_SP_GEO_cells_genes_ML,
                               enrich.databases = c("GO_Biological_Process_2021"),
                               p_adj = 0.05, logFC = 1, save.results = TRUE)
...
Enrich_GEO_CC_ML <- Enrichment(df_SP_GEO_cells_genes_ML,
...
pdf("ML_predicted_SP_GEO_cells_GEO_CC_.pdf")
print(Enrich_GEO_CC_ML_P +ggtitle("SP - GEO CC ML predicted")+ 
        theme(
          legend.title = element_text(size = 6), 
          legend.text = element_text(size = 6)   
        ) )
...
# remove NA values
df_GEO_cells <- df_GEO_cells[!is.na(df_GEO_cells$padj), ]
df_GEO_cells[rownames(df_GEO_cells) == "NGFR", ]
head(df_GEO_cells)
...
SP_GEO_cells <- Gene2SProtein(genes = fdr_GeneID_cells, input_type = "gene_name")
#if we apply filter df_GEO_cells$Mean_CPM_C < 0.5: 77 out of 350 genes have a matching surface protein
# if we apply filter df_GEO_cells$Mean_CPM_C < 0.5: 43 out of 330 genes have a matching surface protein
...
SPup_GEO_cells$GeneID


fdrDW_GeneID_cells <- df_GEO_cells[df_GEO_cells$padj < 0.05 & df_GEO_cells$log2FoldChange < 0, "GeneID"]
SPdw_GEO_cells <- Gene2SProtein(genes = fdrDW_GeneID_cells, input_type = "gene_name")
#The input genes do not have any match in the surfaceome database. 
# if we apply filter df_GEO_cells$Mean_CPM_C < 0.5: 0 genes have a matching surface protein
...
pdf("Almen_class_SPUP_GSE90711.pdf")
Splot(SPup_GEO_cells,
      group.by = "Membranome.Almen.main-class",
      main = "Almen class Adj")
dev.off()
...
dfList <- list( GEO = as.data.frame(df_SP_GEO_cells_genes))
...
filteredDGE <- df_GEO_cells[df_GEO_cells$GeneID %in% SPup_GEO_cells$GeneID,]
...
write.xlsx(list_df, "Supp_table_Annotate_SPID_GSE90711.xlsx")
getwd()
```
